...
When it comes to agency distribution across the main phases of therapy, the findings suggest that therapists do not have a single, stable preference for control in a *Human-AI system*. Instead, their preference varied on in which therapy phase they are in. In the first phase, where understanding the patient motivation, deciding on long-term patient goals, and planning treatment are important, the therapists favored full human agency in reasoning specifically (_S1,S3_). This can be interpreted as therapists' need to reconcile the subjective information from the patient to collaboratively set the goals and plan the entire therapy plan to provide personalized care for that patient. // need studies from patient perspective

When planning the session, therapists' preference shifted to... increasingly accept, and even prefer, system-led reasoning and/or execution (_S2, S3_), as long as they retain the ability to intervene. Participants framed planning as repetitive, especially in a time-constraint situation, provided they could still adjust it. They, similarly, framed executing the clinical decisions as hectic(?) and time-consuming. The system support is seen as valuable for selecting and sequencing exercises consistent with the main therapy goals. // need source to support it 
//This however aligns with their preference for less detail and only focus on higher level therapeutic decisions. 

For rehabilitation sessions itself, participants were most willing to delegate agency in reasoning (_S2_) while some even accepted full system agency (_S4_) choosing to intervene only when necessary. Rehabilitation time is especially cognitively demanding to adjust exergame parameters while simulateneously monitoring patient performance. Adaptive systems supported by ML that can perform Dynamic Difficulty Adjustment (DDA) based on patient performance and other signals could be useful. It aligns with the human-in-the-loop healthcare AI studies where automation is deemed most acceptable when the system manages low-level and continuous tasks while therapists supervise (Olawade et al., 2026; Lazaros et al., 2026).
